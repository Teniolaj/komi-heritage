    ...
    <div className="bg-surface text-on-surface flex min-h-screen font-body">
      ...
      <aside className="bg-stone-950 h-screen w-64 hidden md:flex flex-col fixed left-0 top-0 z-50 p-6 transition-all duration-200 ease-in-out">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link 
                key={link.href}
                href={link.href} 
                className={`flex items-center gap-4 py-3 transition-all duration-200 ease-in-out cursor-pointer group rounded-r-2xl ${
                  isActive 
        ...
        <div className="mt-auto pt-6 border-t border-stone-800/50">
          <button onClick={handleSignOut} className="flex items-center gap-4 px-4 py-3 w-full cursor-pointer text-stone-500 hover:text-primary transition-colors pr-0 pl-0">
            <LogOut size={18} />
            <span className="font-dm-sans uppercase tracking-widest text-[11px] font-bold">Logout</span>
          </button>
          
          <div className="mt-6 flex items-center gap-3 bg-stone-900/50 p-3 -mx-2 rounded-2xl cursor-pointer">
            <div className="w-10 h-10 bg-stone-800 flex items-center justify-center overflow-hidden shrink-0 rounded-full">
      ...
      <div className="flex-1 md:ml-64 bg-surface min-h-screen flex flex-col overflow-hidden pb-16 md:pb-0">
        ...
        <header className="bg-[#fcf9f4]/90 sticky top-0 backdrop-blur-md border-b border-outline-variant/20 flex justify-between items-center w-full px-6 py-4 md:px-8 md:py-6 z-40">
          ...
          <div className="flex items-center gap-4 md:gap-6">
            <div className="hidden lg:flex items-center bg-surface-container px-4 py-2 hover:bg-surface-container-high transition-colors rounded-full cursor-pointer">
            ...
            <div className="flex items-center gap-1 md:gap-2">
              <button className="w-10 h-10 flex items-center justify-center hover:bg-stone-100 transition-colors text-stone-600 rounded-full cursor-pointer">
                <Bell size={20} />
              </button>
              <button className="w-10 h-10 flex items-center justify-center hover:bg-stone-100 transition-colors text-stone-600 md:hidden rounded-full cursor-pointer">
                <UserCircle size={24} />
              </button>
            </div>
          </div>
      ...
      <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center bg-[#fcf9f4] border-t border-stone-200 md:hidden h-[60px] shadow-[0_-4px_24px_rgba(0,0,0,0.06)] px-2 rounded-t-3xl">
        <Link href="/admin" className={`flex flex-col items-center justify-center p-2 h-full w-full duration-100 rounded-xl cursor-pointer ${pathname === '/admin' ? 'bg-primary text-white' : 'text-stone-500 active:bg-stone-200'}`}>
          <Home size={20} />
          <span className="font-dm-sans uppercase text-[9px] font-bold mt-1 tracking-widest">Home</span>
        </Link>
        <Link href="/admin/orders" className={`flex flex-col items-center justify-center p-2 h-full w-full duration-100 rounded-xl cursor-pointer ${pathname === '/admin/orders' ? 'bg-primary text-white' : 'text-stone-500 active:bg-stone-200'}`}>
          <Receipt size={20} />
          <span className="font-dm-sans uppercase text-[9px] font-bold mt-1 tracking-widest">Orders</span>
        </Link>
        <Link href="/admin/menu" className={`flex flex-col items-center justify-center p-2 h-full w-full duration-100 rounded-xl cursor-pointer ${pathname === '/admin/menu' ? 'bg-primary text-white' : 'text-stone-500 active:bg-stone-200'}`}>
          <MenuIcon size={20} />
          <span className="font-dm-sans uppercase text-[9px] font-bold mt-1 tracking-widest">Menu</span>
        </Link>
        <Link href="/admin/more" className={`flex flex-col items-center justify-center p-2 h-full w-full duration-100 rounded-xl cursor-pointer ${pathname === '/admin/more' ? 'bg-primary text-white' : 'text-stone-500 active:bg-stone-200'}`}>
